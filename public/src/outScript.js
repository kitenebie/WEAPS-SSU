...
function updateCoverPhoto() {
    alert("Change cover photo functionality coming soon!");
            ...
            function submitReview(companyId) {
                ...
                if (!ratingEl) {
                    Swal.fire({ icon: 'error', title: 'Error!', text: 'Rating input not found.' });
                    return;
                }
                        ...
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <div class="text-right">
                                ...
                                <div class="flex items-center mt-1">
                                    ...
                                    <span class="ml-2 text-sm text-gray-600">(${reviewCount} ${reviewCount === 1 ? 'review' : 'reviews'})</span>
                                </div>
                            </div>
                        </div>
                        <div class="grid grid-cols-5 gap-2">
                            ${generateRatingDistribution(data.rating_distribution || {}, reviewCount)}
                        </div>
                    </div>
                `;
                                            ...
                                            <span class="text-white font-bold text-sm">
                                                ${
                                                    review.user && !review.is_anonymous
                                                        ? (review.user.first_name ? review.user.first_name.charAt(0) : 'U') +
                                                          (review.user.last_name ? review.user.last_name.charAt(0) : 'U')
                                                        : 'AN'
                                                }
                                            </span>
                                        </div>
            ...
            function initializeStarRating() {
                ...
                stars.forEach((star) => {
                    star.addEventListener('click', function () {
                        ...
                        ratingText.textContent = rating + (rating === 1 ? ' Star' : ' Stars') + ' - ' + ratingLabels[rating];
                    });
            ...
            fetch('/company/review/store', {
                ...
                .catch((error) => {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                        text: error.message || 'An error occurred while submitting the review.',
                    });
                })
                .finally(() => {
                    if (submitBtn) {
