@php
    ...
    if (!$recordActionsAlignment instanceof Alignment) {
        $recordActionsAlignment = filled($recordActionsAlignment)
            ? Alignment::tryFrom($recordActionsAlignment) ?? $recordActionsAlignment
            : null;
    }
...
<div @if (!$isLoaded) wire:init="loadTable" @endif x-data="filamentTable({
    ...
    style="border-width: 2px; border-color: #1E1E1E; border-radius: 1rem;"
    ...
    <div @class(['fi-ta-ctn', 'fi-ta-ctn-with-header' => $hasHeader])>
        <div @if (!$hasHeader) x-cloak @endif
            ...
            @if ($hasFiltersAboveContent)
                <div x-data="{ areFiltersOpen: @js(!$hasFiltersAboveContentCollapsible) }" x-bind:class="{ 'fi-open': areFiltersOpen }" @class(['fi-ta-filters-above-content-ctn'])>
                    <x-filament-tables::filters :apply-action="$filtersApplyAction" :form="$filtersForm" :heading-tag="$secondLevelHeadingTag" x-cloak
                        x-show="areFiltersOpen" />

                    @if ($hasFiltersAboveContentCollapsible)
                        <span x-on:click="areFiltersOpen = ! areFiltersOpen" class="fi-ta-filters-trigger-action-ctn">
                ...
                <div class="fi-ta-actions fi-align-start fi-wrapped">
                    ...
                    @if ($areGroupingSettingsVisible)
                        ...
                        $watch('group', function(newGroup, oldGroup) {
                            ...
                            <x-filament::dropdown placement="bottom-start" shift width="xs"
                                wire:key="{{ $this->getId() }}.table.grouping" @class([
                                ...
                                <div class="fi-ta-grouping-settings-fields">
                                    <label>
                                        ...
                                        <x-filament::input.wrapper>
                                            <x-filament::input.select x-model="group"
                            ...
                            @if (!$areGroupingSettingsInDropdownOnDesktop)
                                <div class="fi-ta-grouping-settings-fields">
                                    <label>
                                        <x-filament::input.wrapper :prefix="__('filament-tables::table.grouping.fields.group.label')">
                                            <x-filament::input.select x-model="group"
                                                x-on:change="resetCollapsedGroups()">
                                                <option value="">-</option>
                ...
                @if ($isGlobalSearchVisible || $hasFiltersDialog || $hasColumnManagerDropdown)
                    <div>
                        ...
                        @if ($isGlobalSearchVisible)
                            ...
                            <x-filament-tables::search-field :debounce="$searchDebounce" :on-blur="$isSearchOnBlur" :placeholder="$searchPlaceholder" />
                        @endif
                        ...
                        @if ($hasFiltersDialog || $hasColumnManagerDropdown)
                            @if ($hasFiltersDialog)
                                @if ($filtersLayout === FiltersLayout::Modal || $filtersTriggerAction->isModalSlideOver())
                                    @php
                                        ...
                                        $filtersTriggerActionModalHeading =
                                            $filtersTriggerAction->getCustomModalHeading() ??
                                ...
                                @else
                                    ...
                                    <x-filament::dropdown :max-height="$filtersFormMaxHeight" placement="bottom-end" shift
                                        :width="$filtersFormWidth ?? Width::ExtraSmall" :wire:key="$this->getId() . '.table.filters'"
                                        class="fi-ta-filters-dropdown">
                                        <x-slot name="trigger">
                                            {{ $filtersTriggerAction->badge($activeFiltersCount) }}
                                        </x-slot>

                                        <x-filament-tables::filters :apply-action="$filtersApplyAction" :form="$filtersForm"
                            ...
                            @if ($hasColumnManagerDropdown)
                                ...
                                <x-filament::dropdown :max-height="$columnManagerMaxHeight" placement="bottom-end" shift :width="$columnManagerWidth"
                                    ...
                                    <x-filament-tables::column-manager :apply-action="$columnManagerApplyAction" :columns="$columnManagerColumns"
                                        :has-reorderable-columns="$hasReorderableColumns" :has-toggleable-columns="$hasToggleableColumns" :heading-tag="$secondLevelHeadingTag" :reorder-animation-duration="$getReorderAnimationDuration()" />
                                </x-filament::dropdown>
                            @endif
        ...
        @if ($isReordering)
            <div x-cloak wire:key="{{ $this->getId() }}.table.reorder.indicator" class="fi-ta-reorder-indicator">
        ...
        @elseif ($isSelectionEnabled && $maxSelectableRecords !== 1 && $isLoaded)
            <div x-cloak x-bind:hidden="!getSelectedRecordsCount()" x-show="getSelectedRecordsCount()"
                wire:key="{{ $this->getId() }}.table.selection.indicator" class="fi-ta-selection-indicator">
                <div>
                    {{ \Filament\Support\generate_loading_indicator_html(
                        new \Illuminate\View\ComponentAttributeBag([
                            'x-show' => 'isLoading',
                ...
                @if (!$isSelectionDisabled)
                    <div>
                        ...
                        <div class="fi-ta-selection-indicator-actions-ctn">
                            <x-filament::link color="primary" tag="button" x-on:click="selectAllRecords"
                                x-show="canSelectAllRecords()" {{-- Make sure the Alpine attributes get re-evaluated after a Livewire request: --}}
                            ...
                            <x-filament::link color="danger" tag="button" x-on:click="deselectAllRecords">
        ...
        @if ($filterIndicators)
            @if (filled(
                    $filterIndicatorsView = FilamentView::renderHook(TablesRenderHook::FILTER_INDICATORS,
                        scopes: static::class,
                        data: ['filterIndicators' => $filterIndicators])))
                ...
                <div class="fi-ta-filter-indicators">
                            ...
                            @foreach ($filterIndicators as $indicator)
                                ...
                                <x-filament::badge :color="$indicatorColor">
                                    ...
                                    @if ($indicator->isRemovable())
                                        ...
                                        <x-slot name="deleteButton" :label="__('filament-tables::table.filters.actions.remove.label')"
                                            :wire:click="$indicatorRemoveLivewireClickHandler"
                                            wire:loading.attr="disabled" wire:target="removeTableFilter"></x-slot>
                                    @endif
                                </x-filament::badge>
                            @endforeach
                        </div>
                    ...
                    @if (collect($filterIndicators)->contains(fn(\Filament\Tables\Filters\Indicator $indicator): bool => $indicator->isRemovable()))
                        <button type="button"
                            ...
                            wire:click="removeTableFilters" wire:loading.attr="disabled"
                            wire:target="removeTableFilters,removeTableFilter" class="fi-icon-btn fi-size-sm">
        ...
        @if ((!$content && !$hasColumnsLayout) || $records === null || count($records))
            <div @if (!$isReordering && ($pollingInterval = $getPollingInterval())) wire:poll.{{ $pollingInterval }} @endif class="fi-ta-content-ctn">
                @if (($content || $hasColumnsLayout) && $records !== null && count($records))
                    @if (!$isReordering)
                        @php
                            ...
                            <div class="fi-ta-content-header">
                                @if ($isSelectionEnabled && $maxSelectableRecords !== 1 && !$isReordering)
                                    <input
                                        ...
                                        type="checkbox"
                                        @if ($isSelectionDisabled) disabled
                                        ...
                                        return null"
                                        x-on:click="toggleSelectRecordsOnPage" {{-- Make sure the "checked" state gets re-evaluated after a Livewire request: --}}
                                ...
                                @if (count($sortableColumns))
                                    ...
                                    $watch('column', function(newColumn, oldColumn) {
                                        ...
                                        <label>
                                            <x-filament::input.wrapper :prefix="__('filament-tables::table.sorting.fields.column.label')">
                    ...
                    @else
                        <div @if ($isReorderable) x-on:end.stop="
                                    $wire.reorderTable(
                                ...
                                "
                                x-sortable
                                data-sortable-animation-duration="{{ $getReorderAnimationDuration() }}" @endif
                            {{ new ComponentAttributeBag()->when($contentGrid, fn(ComponentAttributeBag $attributes) => $attributes->grid($contentGrid))->class([
                                    'fi-ta-content',
                            ...
                            @foreach ($records as $record)
                                ...
                                @if ($recordGroupTitle !== $previousRecordGroupTitle)
                                    @if ($hasSummary && !$isReordering && filled($previousRecordGroupTitle))
                                        <table @class(['fi-ta-table', 'fi-ta-table-reordering' => $isReordering])>
                                            <tbody>
                                                @php
                                                    $groupScopedAllTableSummaryQuery = $group->scopeQuery(
                                                        $this->getAllTableSummaryQuery(),
                                                        $previousRecord,
                                                    );
                                                @endphp

                                                <x-filament-tables::summary.row :columns="$columns" extra-heading-column
                                                    :heading="__('filament-tables::table.summary.subheadings.group', [
                                                        'group' => $previousRecordGroupTitle,
                                                        'label' => $pluralModelLabel,
                                                    ])" :placeholder-columns="false" :query="$groupScopedAllTableSummaryQuery"
                                    ...
                                    <div @if ($isRecordGroupCollapsible = $group->isCollapsible()) x-on:click="toggleCollapseGroup(@js($recordGroupTitle))"
                                            @if (!$hasSummary)
                                                x-bind:class="{ 'fi-collapsed': isGroupCollapsed(@js($recordGroupTitle)) }" @endif
                                        @endif
                                        @class([
                                            'fi-ta-group-header',
                                            ...
                                            <input
                                                ...
                                                @else
                                                    ...
                                                    @if ($maxSelectableRecords)
                                                        ...
                                                        " @endif
                                                @endif
                                            x-bind:checked="const recordsInGroup = JSON.parse($el.dataset.groupSelectableRecordKeys)
                                        ...
                                        @if ($isRecordGroupCollapsible)
                                            <button
                                                ...
                                                type="button" class="fi-icon-btn fi-size-sm">
                                ...
                                <div @if ($hasCollapsibleColumnsLayout) x-data="{ isCollapsed: @js($collapsibleColumnsLayout->isCollapsed()) }"
                                        x-init="$dispatch('collapsible-table-row-initialized')"
                                        x-on:collapse-all-table-rows.window="isCollapsed = true"
                                        x-on:expand-all-table-rows.window="isCollapsed = false"
                                        x-bind:class="isCollapsed && 'fi-ta-record-collapsed'" @endif
                                    wire:key="{{ $this->getId() }}.table.records.{{ $recordKey }}"
                                    @if ($isReordering) x-sortable-item="{{ $recordKey }}"
                                        x-sortable-handle @endif
                                    @class([
                                        'fi-ta-record',
                                        'fi-clickable' => $recordUrl || $recordAction,
                                        'fi-ta-record-with-content-prefix' =>
                                            $isReordering || ($isSelectionEnabled && $isRecordSelectable($record)),
                                        'fi-ta-record-with-content-suffix' =>
                                            $hasCollapsibleColumnsLayout && !$isReordering,
                                    ...
                                    @elseif ($isSelectionEnabled && $isRecordSelectable($record))
                                        <input
                                            ...
                                            @elseif ($maxSelectableRecords && $maxSelectableRecords !== 1)
                                                x-bind:disabled="! areRecordsToggleable([@js($recordKey)])" @endif
                                            value="{{ $recordKey }}"
                                            x-on:click="toggleSelectedRecord(@js($recordKey))"
                                            x-bind:checked="isRecordSelected(@js($recordKey)) ? 'checked' : null"
                                            data-group="{{ $recordGroupKey }}" wire:loading.attr="disabled"
                                            wire:target="{{ implode(',', \Filament\Tables\Table::LOADING_TARGETS) }}"
                                    ...
                                    <div class="fi-ta-record-content-ctn">
                                        <div>
                                            @if ($recordUrl)
                                                <a {{ \Filament\Support\generate_href_html($recordUrl, $openRecordUrlInNewTab) }}
                                                    class="fi-ta-record-content">
                                                    @foreach ($columnsLayout as $columnsLayoutComponent)
                                                        {{ $columnsLayoutComponent->record($record)->recordKey($recordKey)->rowLoop($loop)->renderInLayout() }}
                                                    @endforeach
                                                </a>
                                            @elseif ($recordAction)
                                                @php
                                                ...
                                                <button type="button" wire:click="{{ $recordWireClickAction }}"
                                                    wire:loading.attr="disabled"
                                            ...
                                            @if ($hasCollapsibleColumnsLayout && !$isReordering)
                                                <div x-collapse x-show="! isCollapsed"
                                    ...
                                    @if ($hasCollapsibleColumnsLayout && !$isReordering)
                                        <button type="button" x-on:click="isCollapsed = ! isCollapsed"
                            ...
                            @if (
                                $hasSummary &&
                                    !$isReordering &&
                                    filled($previousRecordGroupTitle) &&
                                    (!$records instanceof \Illuminate\Contracts\Pagination\Paginator || !$records->hasMorePages()))
                    ...
                    @if ($hasSummary && !$isReordering)
                        <table class="fi-ta-table">
                            <tbody>
                                <x-filament-tables::summary :columns="$columns" extra-heading-column :placeholder-columns="false"
                                    :plural-model-label="$pluralModelLabel" :records="$records" />
                            </tbody>
                        </table>
                    @endif
                @elseif (!($content || $hasColumnsLayout) && $records !== null)
                    <table class="fi-ta-table">
                        <thead>
                                ...
                                <tr class="fi-ta-table-head-groups-row">
                                    @if (count($records))
                                        @if ($isReordering)
                                            <th></th>
                                        @else
                                            @if (count($defaultRecordActions) &&
                                                    in_array($recordActionsPosition, [RecordActionsPosition::BeforeCells, RecordActionsPosition::BeforeColumns]))
                                                <th></th>
                                            @endif
                                    ...
                                    @foreach ($columnsLayout as $columnGroup)
                                        ...
                                        @elseif ($columnGroup instanceof ColumnGroup)
                                            ...
                                            @if ($columnGroupColumnsCount)
                                                <th colspan="{{ $columnGroupColumnsCount }}"
                                                    {{ $columnGroup->getExtraHeaderAttributeBag()->class([
                                                            'fi-ta-header-group-cell',
                                                            'fi-wrapped' => $columnGroup->canHeaderWrap(),
                                                            ($columnGroupAlignment = $columnGroup->getAlignment()) instanceof \Filament\Support\Enums\Alignment
                                                                ? "fi-align-{$columnGroupAlignment->value}"
                                                                : (is_string($columnGroupAlignment)
                                                                    ? $columnGroupAlignment
                                                                    : ''),
                                    ...
                                    @if (!$isReordering && count($records))
                                        @if (count($defaultRecordActions) &&
                                                in_array($recordActionsPosition, [RecordActionsPosition::AfterColumns, RecordActionsPosition::AfterCells]))
                                            <th></th>
                                        @endif
                            ...
                            <tr>
                                ...
                                @foreach ($columns as $column)
                                    ...
                                    <th @if ($isColumnActivelySorted) aria-sort="{{ $sortDirection === 'asc' ? 'ascending' : 'descending' }}" @endif
                                        {{ $column->getExtraHeaderAttributeBag()->class([
                                                ...
                                                ($columnAlignment = $column->getAlignment()) instanceof \Filament\Support\Enums\Alignment
                                                    ? "fi-align-{$columnAlignment->value}"
                                                    : (is_string($columnAlignment)
                                                        ? $columnAlignment
                                                        : ''),
                                        ...
                                        @if ($isColumnSortable)
                                            <span aria-label="{{ trim(strip_tags($columnLabel)) }}" role="button"
                                                ...
                                                wire:loading.attr="disabled" class="fi-ta-header-cell-sort-btn">
                                                {{ $columnLabel }}

                                                {{ \Filament\Support\generate_icon_html(
                                                    $isColumnActivelySorted && $sortDirection === 'asc'
                                                        ? \Filament\Support\Icons\Heroicon::ChevronUp
                                                        : \Filament\Support\Icons\Heroicon::ChevronDown,
                                                    alias: match (true) {
                                                        $isColumnActivelySorted && $sortDirection === 'asc'
                                                            => \Filament\Tables\View\TablesIconAlias::HEADER_CELL_SORT_ASC_BUTTON,
                                                        $isColumnActivelySorted && $sortDirection === 'desc'
                                                            => \Filament\Tables\View\TablesIconAlias::HEADER_CELL_SORT_DESC_BUTTON,
                        ...
                        @if ($isColumnSearchVisible || count($records))
                            <tbody
                                @if ($isReorderable) x-on:end.stop="
                                        $wire.reorderTable(
                                            $event.target.sortable.toArray(),
                                            $event.item.getAttribute('x-sortable-item'),
                                        )
                                    ...
                                    <tr class="fi-ta-row fi-ta-row-not-reorderable">
                                        ...
                                        @foreach ($columns as $column)
                                            ...
                                            <td @class([
                                                ...
                                                str($columnName)->camel()->kebab() => $isIndividuallySearchable,
                                            ])>
                                                @if ($isIndividuallySearchable)
                                                    <x-filament-tables::search-field :debounce="$searchDebounce"
                                                        :on-blur="$isSearchOnBlur" :wire-model="'tableColumnSearches.' . $columnName" />
                                                @endif
                                            </td>
                                        @endforeach
                                ...
                                @if (count($records))
                                    ...
                                    @foreach ($records as $record)
                                        ...
                                        @if ($recordGroupTitle !== $previousRecordGroupTitle)
                                            @if ($hasSummary && !$isReordering && filled($previousRecordGroupTitle))
                                                ...
                                                <x-filament-tables::summary.row :actions="count($defaultRecordActions)" :actions-position="$recordActionsPosition"
                                                    :columns="$columns" :group-column="$groupColumn" :groups-only="$isGroupsOnly"
                                                    :heading="$isGroupsOnly
                                                        ? $previousRecordGroupTitle
                                                        : __('filament-tables::table.summary.subheadings.group', [
                                                            'group' => $previousRecordGroupTitle,
                                                            'label' => $pluralModelLabel,
                                                        ])" :query="$groupScopedAllTableSummaryQuery" :record-checkbox-position="$recordCheckboxPosition"
                                                    :selected-state="$groupedSummarySelectedState[$previousRecordGroupKey] ??
                                            ...
                                            @if (!$isGroupsOnly)
                                                <tr class="fi-ta-row fi-ta-group-header-row">
                                                        ...
                                                        if ($isSelectionEnabled) {
                                                            ...
                                                            if (
                                                                $recordCheckboxPosition ===
                                                                    RecordCheckboxPosition::BeforeCells &&
                                                                ...
                                                                <input
                                                                    ...
                                                                    @else
                                                                        ...
                                                                        @if ($maxSelectableRecords)
                                                                            ...
                                                                            " @endif
                                                                    @endif
                                                                x-bind:checked="const recordsInGroup = JSON.parse($el.dataset
                                                                    .groupSelectableRecordKeys)
                                                                
                                                                if (recordsInGroup.length && areRecordsSelected(
                                                                        recordsInGroup)) {
                                                                    $el.checked = true
                                                    ...
                                                    <td colspan="{{ $groupHeaderColspan }}"
                                                        class="fi-ta-group-header-cell">
                                                        <div @if ($isRecordGroupCollapsible) x-on:click="toggleCollapseGroup(@js($recordGroupTitle))"
                                                                x-bind:class="isGroupCollapsed(@js($recordGroupTitle)) ? 'fi-collapsed' : null" @endif
                                                            @class([
                                                                'fi-ta-group-header',
                                                                'fi-collapsible' => $isRecordGroupCollapsible,
                                                            ])>
                                        ...
                                        @if (!$isGroupsOnly)
                                            <tr wire:key="{{ $this->getId() }}.table.records.{{ $recordKey }}"
                                                ...
                                                @if ($isReordering)
                                                    <td class="fi-ta-cell">
                                                        <button class="fi-ta-reorder-handle fi-icon-btn"
                                                            type="button">
                                                            {{ \Filament\Support\generate_icon_html(\Filament\Support\Icons\Heroicon::Bars2, alias: \Filament\Tables\View\TablesIconAlias::REORDER_HANDLE) }}
                                                        </button>
                                                    </td>
                                                    ...
                                                    <td class="fi-ta-cell">
                                                        <div @class([
                                                            ...
                                                            match ($recordActionsAlignment) {
                                                                ...
                                                                default => is_string($recordActionsAlignment)
                                                                    ? $recordActionsAlignment
                                                                    : '',
                                                            },
                                                        ])>
                                                            @foreach ($recordActions as $action)
                                                    ...
                                                    <td class="fi-ta-cell fi-ta-selection-cell">
                                                        @if ($isRecordSelectable($record))
                                                            <input
                                                                ...
                                                                value="{{ $recordKey }}"
                                                                x-on:click="toggleSelectedRecord(@js($recordKey))"
                                                                x-bind:checked="isRecordSelected(@js($recordKey)) ?
                                                                    'checked' : null"
                                                ...
                                                @foreach ($columns as $column)
                                                    @php
                                                        ...
                                                        $isColumnClickDisabled =
                                                            $column->isClickDisabled() || $isReordering;

                                                        $columnWrapperTag = match (true) {
                                                            ($columnUrl || ($recordUrl && $columnAction === null)) &&
                                                                !$columnHasStateBasedUrls &&
                                                                !$isColumnClickDisabled
                                                                => 'a',
                                                            ($columnAction || $recordAction) &&
                                                                !$columnHasStateBasedUrls &&
                                                                !$isColumnClickDisabled
                                                                => 'button',
                                                            default => 'div',
                                                    ...
                                                    <td wire:key="{{ $this->getId() }}.table.record.{{ $recordKey }}.column.{{ $column->getName() }}"
                                                        {{ $column->getExtraCellAttributeBag()->class([
                                                                ...
                                                                ($columnAlignment = $column->getAlignment()) instanceof \Filament\Support\Enums\Alignment
                                                                    ...
                                                                    : (is_string($columnAlignment)
                                                                        ? $columnAlignment
                                                                        : ''),
                                                                ($columnVerticalAlignment = $column->getVerticalAlignment()) instanceof
                                                                \Filament\Support\Enums\VerticalAlignment
                                                                    ? "fi-vertical-align-{$columnVerticalAlignment->value}"
                                                                    : (is_string($columnVerticalAlignment)
                                                        ...
                                                        <{{ $columnWrapperTag }}
                                                            @if ($columnWrapperTag === 'a') {{ \Filament\Support\generate_href_html($columnUrl ?: $recordUrl, $columnUrl ? $column->shouldOpenUrlInNewTab() : $openRecordUrlInNewTab) }}
                                                            @elseif ($columnWrapperTag === 'button')
                                                                type
                                                                ="button"
                                                                wire:click.prevent.stop="{{ $columnWireClickAction }}"
                                                                wire:loading.attr="disabled"
                                                                wire:target="{{ $columnWireClickAction }}" @endif
                                                            @class([
                                                                'fi-ta-col',
                                                                'fi-ta-col-has-column-url' =>
                                                                    $columnWrapperTag === 'a' && filled($columnUrl),
                                                            ])>
                                                            {{ $column }}
                                                            </{{ $columnWrapperTag }}>
                                                    </td>
                                    ...
                                    @if (
                                        ...
                                        <x-filament-tables::summary.row :actions="count($defaultRecordActions)" :actions-position="$recordActionsPosition"
                                            :columns="$columns" :group-column="$groupColumn" :groups-only="$isGroupsOnly" :heading="$isGroupsOnly
                                                ? $previousRecordGroupTitle
                                                : __('filament-tables::table.summary.subheadings.group', [
                                                    'group' => $previousRecordGroupTitle,
                                                    'label' => $pluralModelLabel,
                                                ])"
                                            :query="$groupScopedAllTableSummaryQuery" :record-checkbox-position="$recordCheckboxPosition" :selected-state="$groupedSummarySelectedState[$previousRecordGroupKey] ?? []"
                                            :selection-enabled="$isSelectionEnabled" />
                                    @endif
                                    ...
                                    @if ($hasSummary && !$isReordering)
                                        ...
                                        <x-filament-tables::summary :actions="count($defaultRecordActions)" :actions-position="$recordActionsPosition"
                                            :columns="$columns" :group-column="$groupColumn" :groups-only="$isGroupsOnly" :plural-model-label="$pluralModelLabel"
                                            :record-checkbox-position="$recordCheckboxPosition" :records="$records" :selection-enabled="$isSelectionEnabled" />
                                    @endif
                                @endif
                            </tbody>
                        @endif
        ...
        @if (
            ...
            <x-filament::pagination :extreme-links="$hasExtremePaginationLinks" :page-options="$paginationPageOptions" :paginator="$records" />
        @endif

        @if ($hasFiltersBelowContent)
            <x-filament-tables::filters :apply-action="$filtersApplyAction" :form="$filtersForm" :heading-tag="$secondLevelHeadingTag"
